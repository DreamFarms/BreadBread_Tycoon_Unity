...
[System.Serializable]
public class Node // 타일 하나하나를 의미하는 클래스
{
    public bool isWall; // 벽인지
    public Node ParentNode; // 경로 추적을 위해 이전 노드 저장
...
public class GameManager_A_StartTest : MonoBehaviour
{
    ...
    public List<Node> FinalNodeList; // 최단 경로를 담는 리스트
    public bool allowDiagonal, dontCrossCorner; // 대각선 허용, 대각선 허용하되 모서리 닿기 여부

    int sizeX, sizeY;
    Node[,] NodeArray;
    Node StartNode, TargetNode, CurNode;
    List<Node> OpenList, ClosedList; // 갈 수 있는 곳, 갈 수 없는 곳을 담는 리스트
    ...
    public void PathFinding()
    {
        ...
        NodeArray = new Node[sizeX, sizeY]; // 맵 크기를 담은 2차원 배열
        ...
        OpenList = new List<Node>() { StartNode }; // 탐색 좌표
        ClosedList = new List<Node>(); // 탐색 끝난 좌표
        FinalNodeList = new List<Node>(); // 최종 좌표
